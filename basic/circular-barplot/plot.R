...
#####################################
{
  ...
  if (label) {
    # 添加标签注释信息
    p <- p +
      geom_text(data = label_data, aes(x = id, y = value + 8, label = individual, hjust = hjust), color = "black", fontface = "bold", alpha = 0.6, size = label_size, angle = label_data$angle, inherit.aes = FALSE) +
